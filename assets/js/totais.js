  ...
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     function carregar_lotes() {
         //Ler arquivo LOTE.json
            ...
            var jsonVendas = alasql("SELECT [1]->LOTE_SEXO as LOTE_SEXO, \
                                    sum(CAST([1]->LOTE_QTDE AS INT)) AS LOTE_QTDE, \
                                    sum(CAST(replace(replace(replace([1]->LOTE_VLRTOT, 'R$ ', ''), '.', ''), ',', '.') AS FLOAT)) as LOTE_VLRTOT, \
                                    round(sum(CAST(replace(replace(replace([1]->LOTE_VLRTOT, 'R$ ', ''), '.', ''), ',', '.') AS FLOAT)) / sum(CAST([1]->LOTE_QTDE AS INT)),2) as LOTE_MEDIA \
                                    FROM ? \
                                    WHERE [1]->LOTE_ORDEMBOOK <> '' \
                                    GROUP BY [1]->LOTE_SEXO \
                                    ORDER BY LOTE_SEXO", [json]);
                                    
            var jsonTotais = alasql("SELECT sum(CAST([1]->LOTE_QTDE AS INT)) AS LOTE_QTDE, \
                                    sum(CAST(replace(replace(replace([1]->LOTE_VLRTOT, 'R$ ', ''), '.', ''), ',', '.') AS FLOAT)) as LOTE_VLRTOT, \
                                    round(sum(CAST(replace(replace(replace([1]->LOTE_VLRTOT, 'R$ ', ''), '.', ''), ',', '.') AS FLOAT)) / sum(CAST([1]->LOTE_QTDE AS INT)),2) as LOTE_MEDIA \
                                    FROM ? \
                                    WHERE [1]->LOTE_ORDEMBOOK <> '' ", [json]);

            var jsonMedias = alasql("SELECT [1]->LOTE_SUBTOT as LOTE_SUBTOT, \
                                    sum(CAST([1]->LOTE_QTDE AS INT)) AS LOTE_QTDE, \
                                    sum(CAST(replace(replace(replace([1]->LOTE_VLRTOT, 'R$ ', ''), '.', ''), ',', '.') AS FLOAT)) as LOTE_VLRTOT, \
                                    round(sum(CAST(replace(replace(replace([1]->LOTE_VLRTOT, 'R$ ', ''), '.', ''), ',', '.') AS FLOAT)) / sum(CAST([1]->LOTE_QTDE AS INT)),2) as LOTE_MEDIA \
                                    FROM ? \
                                    WHERE [1]->LOTE_ORDEMBOOK <> '' \
                                    GROUP BY [1]->LOTE_SUBTOT \
                                    ORDER BY LOTE_SUBTOT", [json]);
         
            var index = 0;

            //Popular a tela
            var tbody = $('#vendasleilao');
            
            jsonVendas.forEach(function(element) {
                var tr = $("<tr><td><ul id='tot'></ul></td>" +
                    "<td><ul id='qtde'></ul>" +
                    "<td><ul id='media'></ul>" +
                    "<td><ul id='vlrtot'></ul>");
                ...
                tr.find('#tot')
                .attr('id', 'tot' + index)
                .text(element.LOTE_SEXO);
                tr.find('#qtde')
                .attr('id', 'qtde' + index)
                .text(element.LOTE_QTDE);
                tr.find('#media')
                .attr('id', 'media' + index)
                .text(element.LOTE_MEDIA.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 2 }));
                
                tr.find('#vlrtot')
                .attr('id', 'vlrtot' + index)
                .text(element.LOTE_VLRTOT.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 2 }));
               
                
                tr.appendTo(tbody);
                
                if (jsonVendas.length == index+1) {
                    
                    var tr2 = $("<tr class='tile-header bg-blue'><td><ul id='tot'></ul></td>" +
                    "<td><ul id='qtde'></ul>" +
                    "<td><ul id='media'></ul>" +
                    "<td><ul id='vlrtot''></ul>");
                    ...
                    .attr('id', 'qtdeTotal')
                    .attr('style', 'font-weight: bold; font-size: 15px;')
                    .text(jsonTotais[0].LOTE_QTDE);
                    tr2.find('#media')
                    .attr('id', 'mediaTotal')
                    .attr('style', 'font-weight: bold; font-size: 15px;')
                    .text(jsonTotais[0].LOTE_MEDIA.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 2 }));
                    tr2.find('#vlrtot')
                    .attr('id', 'vlrtotTotal')
                    .attr('style', 'font-weight: bold; font-size: 15px;')
                    .text(jsonTotais[0].LOTE_VLRTOT.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 2 }));
            ...
            var tbody = $('#mediasleilao');
            
            jsonMedias.forEach(function(element) {
                var tr3 = $("<tr class='active'><td><ul id='tot'></ul></td>" +
                    "<td><ul id='qtde'></ul>" +
                    "<td><ul id='media'></ul>" +
                    "<td><ul id='vlrtot' class='currency'></ul>");
                
                
                tr3.find('#tot')
                .attr('id', 'tot' + index)
                .attr('style', 'font-weight: bold; font-size: 15px;')
                .text('Média Geral ' + element.LOTE_SUBTOT);
                tr3.find('#qtde')
                .attr('id', 'qtde' + index)
                .attr('style', 'font-weight: bold; font-size: 15px;')
                .text(element.LOTE_QTDE);
