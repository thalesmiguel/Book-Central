  ...
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    function adicionar_lote() {
        ...
        tr.find('#lote')
        .attr('id', 'lote' + index)
        .attr('onblur', "gravar_lote('LOTE_COD', " + index + ", " + "'lote" + index + "')");
        
        tr.find('#vendedor')
        .attr('id', 'vendedor' + index)
        .attr('onkeyup', "gravar_lote('LOTE_VENDEDOR', " + index + ", " + "'vendedor" + index + "')");
     ...
     function gravar_lote(campo, id, elemento) {
        ...
        if (json[id] == undefined) {
            json[id] = {'LOTE_COD': document.getElementById('lote' + id).value};
            console.log(json[id]);
        //Criar Node
        }
        json[id][campo] = document.getElementById(elemento).value;
     ...
     function carregar_lotes() {
            ...
            var file = './LOTE.json';
            try {
                 json2 = jsonfile.readFileSync(file);
            } catch (error) {
                var objson = xml2json.fromFile('lote.xml');
                var json2 = objson.DATAPACKET.ROWDATA.ROW;
                
                for (var i = 0; i < json2.length ; i++) {
                    obj.push(json2[i]['@attributes']);
                    obj[i].LOTE_ORDEMBOOK = '';
                    obj[i].LOTE_VLRTOT = '';
                }
            ...
            json.forEach(function(element) {
                ...
                tr.find('#lote')
                .attr('id', 'lote' + index)
                .val(element.LOTE_COD);
                
                tr.find('#vendedor')
                .attr('id', 'vendedor' + index)
                .val(element.LOTE_VENDEDOR);
                
                tr.find('#tot')
                .attr('id', 'tot' + index)
                .attr('onkeyup', "gravar_lote('LOTE_SEXO', " + index + ", " + "'tot" + index + "')")
                .val(element.LOTE_SEXO);
                
                if (element.LOTE_SUBTOT == undefined) {
                    element.LOTE_SUBTOT = "";
                }
                tr.find('#subtot')
                .attr('id', 'subtot' + index)
                .attr('onkeyup', "gravar_lote('LOTE_SUBTOT', " + index + ", " + "'subtot" + index + "')")
                .val(element.LOTE_SUBTOT);
                
                tr.find('#qtde')
                .attr('id', 'qtde' + index)
                .attr('onkeyup', "gravar_lote('LOTE_QTDE', " + index + ", " + "'qtde" + index + "')")
                .val(element.LOTE_QTDE);
                
                tr.find('#captacoes')
                .attr('id', 'captacoes' + index)
                .attr('onkeyup', "gravar_lote('LOTE_CAPTACOES', " + index + ", " + "'captacoes" + index + "')")
                .val(element.LOTE_CAPTACOES);
